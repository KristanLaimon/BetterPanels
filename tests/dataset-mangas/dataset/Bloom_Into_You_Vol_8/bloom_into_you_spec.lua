...
---
--- Validates 100% of panel annotations across all 213 pages from json, ensures 'en' language paths,
...
describe("Bloom Into You human-annotated dataset validation", function()
    local available_images = countAvailableImages(BOOK_DIR, 213)
    local is_full_volume = (available_images == 213)

    if not is_full_volume then
        print(
            string.format(
                "\n  [WARNING] %s: Only %d/213 pages found on disk (public clone / DMCA preview mode).",
                BOOK_TITLE,
                available_images
            )
        )
    ...
    it("verifies 100% of panel annotations across all 213 pages in JSON", function()
        ...
        assert.equals(213, #target_book.pages, "Must contain exactly 213 pages in JSON")
        ...
        for _, page in ipairs(target_book.pages) do
            ...
            assert.is_true(p_idx >= 1 and p_idx <= 213, "Page index must be between 1 and 213")
        ...
        -- Ensure every single page from 1 to 213 was covered with no gaps
        for i = 1, 213 do
    ...
    it("verifies raw annotation.json uses 'en' key instead of 'ja'", function()
        ...
        for _, p in ipairs(pages) do
            ...
            assert.is_true(
                img_paths.en:find("^Bloom_Into_You_Vol_8/%d%d+%.png$") ~= nil,
                "image_paths.en must match pattern Bloom_Into_You_Vol_8/XX.png (got " .. tostring(img_paths.en) .. ")"
            )
        end
    end)
    ...
    it("checks page image files on disk (full volume or preview mode)", function()
        if is_full_volume then
            -- Full volume available: check all 213 images on disk
            for p_idx = 1, 213 do
    ...
    it("evaluates panel recognition on available Bloom Into You pages", function()
        local max_test_pages = is_full_volume and 213 or math.min(3, available_images)
